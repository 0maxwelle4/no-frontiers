...
export class AuthService {
  ...
  auth0 = new auth0.WebAuth({
    ...
    // redirectUri: 'https://0maxwelle4.github.io/no-frontiers/',
    redirectUri: 'http://localhost:4200',
    audience: 'https://dev-u5hvg8wb8m85i4jl.eu.auth0.com/api/v2/',
    scope: 'openid profile email read:current_user user_metadata create:users update:users update:users_app_metadata update:current_user_metadata'
  });
  ...
  updatePicture(id: any, pictureUrl: any) {
    return from(this.tokenApi()).pipe(
      switchMap(accessToken => {
        //console.log(accessToken);
  ...
  updateProfesorDesc(id: any, newDesc: any) {
    return from(this.tokenApi()).pipe(
      switchMap(accessToken => {
        //console.log(accessToken);
  ...
  sendEmail(templateId: string, templateParams: any) {
    return emailjs.send('service_orvenuu', templateId, templateParams, 'N8OI897r4wP6q-f5G');
  }

  tokenApi(): Promise<string | void> {
    let accessToken = '';
